...
\begin{itemize}
    \item \textbf{Provider:} Deutsche Bundesbank SDMX REST API \\(\inlinecode{D.I.ZST.ZI.EUR.S1311.B.A604}; \cite{bundesbank_zero_coupon}).
    \item \textbf{Instrument set:} Zero-coupon yield curve for central government debt denominated in EUR.
    \item \textbf{Tenor breadth:} Residual maturities from 0.5 years (\inlinecode{R005X}) through 30 years (\inlinecode{R30XX}), covering each annual bucket in between. This satisfies the assignment requirement to span short- through long-term maturities (see also \cite{bundesbank_metadata}).
\end{itemize}
...
\begin{enumerate}
    ...
    \item For each maturity code in the published Bundesbank metadata, construct the full SDMX series key and request the corresponding CSV slice for the target date \parencite{bundesbank_metadata}.
